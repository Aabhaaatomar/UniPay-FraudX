...
def inject_custom_css(theme):
    ...
    st.markdown("""
        ...
        html, body, [data-testid="stAppViewContainer"], .metric-title, .metric-value, p, h1, h2, h3, label { 
        ...
        .stAlert { margin-top: 1rem !important; margin-bottom: 1rem !important; }
        </style>
...
# Initial attempts to look for local assets
df = load_data()
model = load_model()

# INTERACTIVE RECOVERY FLOW
if df.empty or model is None:
    inject_custom_css("Dark") # Force fallback dark layout configuration
    
    st.markdown("""
        <div style='text-align:center; padding:20px 0 40px 0;'>
            <h1 style='font-size:2.4rem; color: #ff4b8b !important;'>⚠️ System Core Assets Missing</h1>
            <p style='opacity:0.8; max-width:650px; margin:0 auto;'>UniPay FraudX requires an underlying transaction dataset and a trained pipeline model pkl file to initialize the engine.</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.markdown("### 📊 Upload Transaction Registry")
        st.caption("Expected location: `/dataset/data.xlsx`")
        uploaded_df = st.file_uploader("Choose Excel Dataset File", type=["xlsx"], key="setup_dataset")
        if uploaded_df is not None:
            os.makedirs(DATASET_DIR, exist_ok=True)
            with open(os.path.join(DATASET_DIR, "data.xlsx"), "wb") as f:
                f.write(uploaded_df.getbuffer())
            st.success("Dataset written successfully!")
            st.cache_data.clear() # Clear cache memory to force a fresh data read
            st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
        
    with col2:
        st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
        st.markdown("### 🔮 Upload Prediction Weight Matrices")
        st.caption("Expected location: `/models/fraud_model.pkl`")
        uploaded_model = st.file_uploader("Choose Model Pickle File", type=["pkl"], key="setup_model")
        if uploaded_model is not None:
            os.makedirs(MODELS_DIR, exist_ok=True)
            with open(os.path.join(MODELS_DIR, "fraud_model.pkl"), "wb") as f:
                f.write(uploaded_model.getbuffer())
            st.success("Model binaries loaded!")
            st.cache_resource.clear() # Clear model cache reference memory
            st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)

# Run the normal app system ONLY if assets are verified and present
else:
    # Existing sidebar, navigation, layout, and pages code go here...
